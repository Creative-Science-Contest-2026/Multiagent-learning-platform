...
| Screenshot bundle | Browser capture after smoke when the UI changes | Current | Fresh browser recapture completed on 2026-04-28 in `docs/post-phase2-browser-recapture-run` after the Phase 2 polish train. |
| Hybrid authoring screenshots (`/agents`) | Browser capture after smoke when hybrid story is presented | Current | Fresh browser recapture completed on 2026-04-28 in `docs/post-phase2-browser-recapture-run`, including the updated `/agents` adaptive wording. |
...
- The 2026-04-28 smoke pass verified seeded demo-safe assessment and tutor sessions plus Knowledge Pack grounding through API responses, and the same date now also has a fresh browser recapture for the judge-facing Knowledge, Tutor, Dashboard, and `/agents` rows.
...
- Screenshot refresh follow-up:
  - the browser recapture lane `docs/post-phase2-browser-recapture-run` refreshed the Knowledge Pack, Tutor, Dashboard, and `/agents` screenshots on 2026-04-28;
...
5. smoke-backed command evidence is current on 2026-04-28, and the browser-captured Knowledge, Tutor, Dashboard, and `/agents` rows were refreshed the same day against the current Phase 2 polished UI;
6. this evidence set now proves both the command-backed Knowledge Pack -> Assessment session -> Tutor session -> Dashboard path and the latest judge-facing browser UI for the refreshed rows;
...
| Ask Tutor Agent follow-up | Student prompt is visible before the tutor answer | Passed | [`06-tutor-agent-answer.png`](./screenshots/06-tutor-agent-answer.png) |
| Review tutor answer | Tutor responds to student question with learning context | Passed | [`06-tutor-agent-answer.png`](./screenshots/06-tutor-agent-answer.png) |
...
6. Re-run the browser recapture lane if any future UI change makes the current screenshot rows misleading again.
