...
async def get_memory(current_user: AuthenticatedUser = Depends(get_current_user)):
    return _snap_dict(get_memory_service().read_snapshot(owner_user_id=current_user.id))
...
async def update_memory(
    ...
    snap = get_memory_service().write_file(
        payload.file,
        payload.content,
...
async def refresh_memory(
    ...
    snap = get_memory_service().read_snapshot(owner_user_id=current_user.id)
...
async def clear_memory(
    ...
    if target:
        snap = svc.clear_file(target, owner_user_id=current_user.id)
    else:
        snap = svc.clear_memory(owner_user_id=current_user.id)
