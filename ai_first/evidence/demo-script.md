...
For the 2026-04-28 evidence state, command-backed smoke is current and the judge-facing Knowledge, Tutor, Dashboard, and `/agents` screenshots were freshly recaptured against the current merged UI. Diagnosis/intervention still rely on the documented dashboard and case-study evidence rather than a separate browser-only capture.
