...
- Done: Wrote the bounded task packet `docs/superpowers/tasks/2026-04-30-code-review-graph-integration.md` for installing `code-review-graph`, wiring Codex integration, and committing `.code-review-graph/` if the upstream tool creates it locally.
...
- Blockers: none
- Next: stage the bounded repo-local artifact set, commit the lane, and decide whether to keep the root `.gitignore` rule or replace it with a narrower tracked exception in a follow-up if the team wants future graph DB updates auto-visible in `git status`.
