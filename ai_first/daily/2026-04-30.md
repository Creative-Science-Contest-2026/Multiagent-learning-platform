...
- Done: Added one explicit classroom case-study frame centered on the demo-safe `contest-demo-quadratics` flow across `docs/contest/README.md`, `docs/contest/DEMO_SCRIPT.md`, `ai_first/competition/product-description.md`, and `ai_first/competition/pitch-notes.md`.
- Done: Added a compact bounded metric card to the contest docs so reviewers can cite smoke-backed operational proof without drifting into benchmark, learning-gain, or classroom-outcome claims.
- Done: Updated `docs/contest/CASEPACK_AND_EVALUATION_DATASET.md` and `docs/contest/VALIDATION_REPORT.md` so the narrative and metric set point to the same evidence-backed classroom scenario.
- Done: Moved `C219` to `in-progress` in `ai_first/TASK_REGISTRY.json` while this branch is under review.
- Tests: `python3 -m json.tool ai_first/TASK_REGISTRY.json >/dev/null`; `git diff --check`; `rg -n "case study|metric|bounded|non-overclaim|grounding|contest-demo-quadratics|validated prototype|not a classroom outcome" docs/contest ai_first/competition docs/superpowers/tasks docs/superpowers/pr-notes`
- Blockers: none
- Next: open the draft PR with this docs-only packet once human review of the narrative pass is complete.
