...
- Done: Reorganized `Bảng điều khiển giáo viên` into a decision-first layout with a separate KPI row, an intervention-heavy left column, a supporting topic/group summary column, and a calmer lower history/filter layer.
- Done: Reworked `TeacherInsightPanel` and `StudentInsightCard` so the default teacher view now emphasizes who needs attention, what topic is blocked, and what move comes next before revealing system detail.
- Done: Added presenter helpers that hide raw unified student IDs and map teacher-facing status/debug tokens into bounded Vietnamese labels.
- Done: Simplified `SmallGroupInsightCard` copy so grouped intervention language stays teacher-friendly if that component is reused.
- Tests: `cd web && node --test tests/teacher-dashboard-copy.test.ts tests/teacher-dashboard-decision-flow.test.ts`; `git diff --check`
- Notes: targeted eslint and full `next build` are blocked in this worktree because `web/node_modules/.bin/eslint` is unavailable and `next` is not installed in PATH.
