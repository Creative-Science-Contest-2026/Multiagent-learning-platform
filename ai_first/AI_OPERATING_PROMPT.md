...
- Latest operating status: `ai_first/EXECUTION_QUEUE.md` is the shortest queue/status board, the post-`T042` wait-state sync merged into `main` through PR `#111`, the repository already contains the manual-review read path, and the current derived short task is `T043 Optional Contest Video Capture Runbook` on branch `docs/t043-video-capture`.
