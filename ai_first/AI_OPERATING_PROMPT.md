...
- Latest operating status: `ai_first/EXECUTION_QUEUE.md` is the shortest queue/status board, `T043 Optional Contest Video Capture Runbook` merged into `main` through PR `#113`, the repository now contains both the manual-review read path and the optional video runbook, and the contest submission queue is again waiting on human review rather than an active AI lane.
