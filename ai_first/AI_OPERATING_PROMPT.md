...
- Latest operating status: `ai_first/EXECUTION_QUEUE.md` is the shortest queue/status board, `ai_first/ACTIVE_ASSIGNMENTS.md` is the active coordination board, the full `F101-F124` backlog is merged on `main`, submission-close Phase 1 (`C201-C210`) is merged on `main` through PRs `#210`, `#212`, and `#211`, optional Phase 2 polish (`C211-C215`) is merged on `main` through PRs `#219`, `#214`, `#215`, `#216`, and `#217`, the optional browser screenshot recapture follow-up is merged through PR `#221`, and the post-merge control-plane sync is merged through PR `#222`. Command-backed evidence remains current on 2026-04-28, but the authoritative contest evidence docs still mark browser-backed Knowledge, Tutor, Dashboard, and `/agents` screenshot rows stale until a fresh recapture replaces them. The current repo-level state is human review plus that browser-freshness follow-up, optional video, or another newly opened packet from `main`.
